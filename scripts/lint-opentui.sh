...
if grep -rn --include="*.tsx" --include="*.ts" -E '[^r]color=' src/frontends/tui/ 2>/dev/null \
   | grep -v 'borderColor' \
   | grep -v '//' \
   | grep -v '\.test\.' \
   | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" -E '[^r]color=' src/frontends/tui/ 2>/dev/null \
    | grep -v 'borderColor' \
    | grep -v '//' \
    | grep -v '\.test\.'
...
if grep -rn --include="*.tsx" --include="*.ts" -E '[ ]+(bold|dimmed|italic)[ >={]' src/frontends/tui/ 2>/dev/null \
   | grep -v '//' \
   ...
   | grep -v '/\*' \
   | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" -E '[ ]+(bold|dimmed|italic)[ >={]' src/frontends/tui/ 2>/dev/null \
    | grep -v '//' \
    | grep -v 'TextAttributes' \
    | grep -v '\.test\.' \
    | grep -v ' \* ' \
...
if grep -rn --include="*.tsx" --include="*.ts" -E 'fg=\{[0-9]' src/frontends/tui/ 2>/dev/null | grep -v '//' | grep -v ' \* ' | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" -E 'fg=\{[0-9]' src/frontends/tui/ 2>/dev/null | grep -v '//' | grep -v ' \* '
...
if grep -rn --include="*.tsx" --include="*.ts" -E 'color=\{[0-9]' src/frontends/tui/ 2>/dev/null | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" -E 'color=\{[0-9]' src/frontends/tui/ 2>/dev/null
...
if grep -rn --include="*.tsx" --include="*.ts" 'await render(' src/frontends/tui/ 2>/dev/null | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" 'await render(' src/frontends/tui/ 2>/dev/null
...
if grep -rn --include="*.tsx" --include="*.ts" 'dims()?.columns' src/frontends/tui/ 2>/dev/null | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" 'dims()?.columns' src/frontends/tui/ 2>/dev/null
  echo "ERROR: Use dims()?.width instead of dims()?.columns"
  errors=1
fi

# Check for scrollToEnd() (correct is scrollBy() or scrollTo()).
if grep -rn --include="*.tsx" --include="*.ts" 'scrollToEnd()' src/frontends/tui/ 2>/dev/null | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" 'scrollToEnd()' src/frontends/tui/ 2>/dev/null
...
if grep -rn --include="*.tsx" -E 'Show when=\{.*\(\) &&[^}]+\}>\{\(' src/frontends/tui/ 2>/dev/null \
   | grep -v '//' \
   | grep -v '\.test\.' \
   | grep -q .; then
  grep -rn --include="*.tsx" -E 'Show when=\{.*\(\) &&[^}]+\}>\{\(' src/frontends/tui/ 2>/dev/null \
    | grep -v '//' \
    | grep -v '\.test\.'
...
if grep -rn --include="*.tsx" --include="*.ts" -E 'fg="(gray|white|red|green|yellow|blue)"' src/frontends/tui/ 2>/dev/null \
   | grep -v '//' \
   | grep -v '\.test\.' \
   | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" -E 'fg="(gray|white|red|green|yellow|blue)"' src/frontends/tui/ 2>/dev/null \
    | grep -v '//' \
    | grep -v '\.test\.'
...
if grep -rn --include="*.tsx" --include="*.ts" 'colors\.text\.subtle' src/frontends/tui/ src/commands/ src/storybook/ 2>/dev/null \
   | grep -v 'syntax\.ts' \
   | grep -v 'tokens\.ts' \
   | grep -v '//' \
   | grep -v ' \* ' \
   | grep -v '\.test\.' \
   | grep -q .; then
  grep -rn --include="*.tsx" --include="*.ts" 'colors\.text\.subtle' src/frontends/tui/ src/commands/ src/storybook/ 2>/dev/null \
    | grep -v 'syntax\.ts' \
    | grep -v 'tokens\.ts' \
    | grep -v '//' \
    | grep -v ' \* ' \
...
for_signal_reread=$(grep -rn --include="*.tsx" '<For each=' src/frontends/tui/ src/commands/ 2>/dev/null \
