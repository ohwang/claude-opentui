...
export function getCrossagentSdkMcpConfig(): McpSdkServerConfigWithInstance | null {
  ...
  _sdkConfig = createSdkMcpServer({
    ...
    tools: [
      tool(
        "crossagent_spawn",
        "Spawn a new cross-backend subagent. The subagent runs asynchronously — use crossagent_status to check progress. Use this for delegating tasks to specific backends (e.g., spawn a Gemini subagent for research while you continue working). For same-backend delegation within your own workflow, use your native Agent tool instead.",
        {
